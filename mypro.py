...
def func2():
	...
	for line in fileinput.input("som/output_si.txt"):
		...
		somlist.append([str(i).zfill(4), px, py])
		i += 1
	fileinput.close()
...
def func3():
	out = open("data/move.txt","w")
	for filename in sorted(glob.glob(r"data/*_user_trace.txt")):
		print filename
		date = filename.split("/")[1].split("_")[0]
		status = 1 if date in ["0820","0821","0822","0823","0824"] else 0
		for line in fileinput.input(filename):
			out.write(str(status)+" "+" ".join(line.strip().split(" ")[1:])+"\n")
		fileinput.close()
	out.close()
	pass
