...
file = open("wordmap.txt","w")
file.write(str(len(lda.wordmap()))+"\n")
...
for i in xrange(500):
    print i
    lda.next()
file = open("model-final.phi","w")
for item in lda.estimate_phi():
    file.write(" ".join([format(i,'.6f') for i in item])+"\n")
file.close()
file = open("model-final.theta","w")
